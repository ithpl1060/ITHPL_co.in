$('#uploadCakeDetailForm').on('submit', function (e) {
    ...
    if (returnVal) {
        $.ajax({
            ...
            url: base_url+'uploadcakeimages_api',
...
function uploadCakeImages(detailid) {
    ...
    if(cdetail.img1!=null){
        $("#image1pre").attr("src", base_url+'/'+cdetail.img1);
    }else{
        $("#image1pre").attr("src", base_url+defaultImg);
    }
    if(cdetail.img2!=null){
        $("#image2pre").attr("src", base_url+'/'+cdetail.img2);
    }else{
        $("#image2pre").attr("src", base_url+defaultImg);
    }
    if(cdetail.img3!=null){
        $("#image3pre").attr("src", base_url+'/'+cdetail.img3);
    }else{
        $("#image3pre").attr("src", base_url+defaultImg);
    }
