                ...
                if (response.status == 200) {
                    ...
                    if (response.data.lenght != 0) {
                        for (var i = 0; i < response.data.length; i++) {
                            profileTabList.set(response.data[i].tab_id, response.data[i]);
                        }
...
function setProfileTabList(list) {
    ...
    for (let k of list.keys()) {
        ...
        <a href="#" id="ctrl_`+tab.tab_id+`" onclick="deleteProfileTab(`+tab.tab_id+`)" title="Delete Tab" >`+tab.tab_name+`&nbsp;&nbsp;<i class="fa fa-trash text-danger"></i></a>
        </div>`;
